...
#### 推理效率（批量推理，batch=4，BF16，RTX 5060 Ti 16GB）

| 指标 | 基座模型（Qwen3-4B） | 微调模型（SFT）| 提升 |
|------|---------------------|--------------|------|
| **总耗时**（481 条）| ~57 min | **~17 min** | 3.4× 更快 |
| **平均速度** | ~7.1 s/条 | **~2.1 s/条** | 3.4× 更快 |
| **吞吐量** | ~8.5 条/min | **~28.3 条/min** | 3.4× 更高 |
| **平均输出长度** | ~950 字符（512 token 截断）| **~351 字符** | 减少 63% |

> ⚠️ 基座模型推理时间为受限测量值（max_new_tokens=512，大量样本被截断）。由于基座模型自发触发思考链（`<think>` 块），实际可用 token 预算全被推理过程消耗，结构化答案未能完整输出。在完整配置（max_new_tokens=2048）下实际耗时将更长，推理效率差距进一步扩大。
...
3. **推理效率大幅提升**：SFT 后模型学会了直接输出结构化答案（不触发思考链），平均输出从 ~950 字符缩短至 ~351 字符，批量推理吞吐量提升 **3.4×**（28.3 vs 8.5 条/min）。这一收益来源于模型行为的根本改变，而非工程优化，因此在任何推理框架（HuggingFace / vLLM）下均可复现
4. **思考模式抑制是隐性收益**：Qwen3-4B 基座模型即使在配置 `enable_thinking=false` 的情况下仍自发输出 `<think>` 块，说明基座权重中固化了推理链生成模式。SFT 训练标签中不含 `<think>` 内容，模型梯度更新后学会了直接输出答案，这一指令遵循能力本身即是可量化的微调收益
5. **Bad cases 极少**：微调后仅 2 条格式缺失（0.4%），需后续人工核查是否为极短新闻等边界输入
